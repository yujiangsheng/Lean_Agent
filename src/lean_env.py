...
本模块负责与 Lean 4 / LeanDojo 进行交互，提供：
- 证明状态管理（proof state）
- Tactic 执行与验证
- 定理/引理检索

依赖：
- lean-dojo >= 1.8.0（可选，无时使用 Mock 模式）
...
try:
    from lean_dojo import LeanGitRepo, Dojo, trace
...
def create_lean_env(use_mock: bool = True) -> LeanEnvironment:
    ...
    """
    return LeanEnvironment(use_mock=use_mock)
